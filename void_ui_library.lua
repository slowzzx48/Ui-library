...
local function enableDragging(frame)
    ...
    frame.InputBegan:Connect(function(input)
        if isResizingGlobal then return end
...
function UI:CreateWindow(Config)
    local Window = {
        Name = Config.Name or "Void Library",
        isDestroyed = false,
    ...
    local USER_ZONE = 62 -- user card + gap (keeps scroll clear of avatar)
    ...
    UserDivider.BackgroundTransparency = 1
    --endUser
    ...
    function Window:Tab(Config, type)
       ...
       local function SelectTab()
        ...
        function Window:Anonymous(Value)
            ...
            UserTitle.Text = Value and "Anonymous" or (lp and lp.DisplayName or "Player")
            UserSub.Text = Value and "@Anonymous" or ("@" .. (lp and lp.Name or "Player"))
            local ok, thumb = pcall(function()
                return game:GetService("Players"):GetUserThumbnailAsync(
                    Value and 1 or (lp and lp.UserId or 1),
                    Enum.ThumbnailType.HeadShot,
                    Enum.ThumbnailSize.Size150x150
                )
            end)
            if ok and UserFrame:FindFirstChild("ImageLabel") then
                UserFrame.ImageLabel.Image = thumb
            end
        end
    ...
    local function safeMinimize()
        if Window.isDestroyed or isAnimatingWindow or not Window.IslandOpen then return end
    ...
    local function safeOpen()
        if Window.isDestroyed or isAnimatingWindow or Window.IslandOpen then return end
        isAnimatingWindow = true
        task.spawn(function()
    ...
    local MinimizeBtn = WinElements:FindFirstChild("Minimize") or WinElements:FindFirstChild("ImageButton")
    if MinimizeBtn then
        MinimizeBtn.MouseButton1Click:Connect(safeMinimize)
        MinimizeBtn.Activated:Connect(safeMinimize)
    end

    MinzUI.MouseButton1Click:Connect(safeOpen)
    MinzUI.Activated:Connect(safeOpen)

    function Window:OnDestroy(Callback)
        Window.OnDestroy = Callback or function() end
    end

    local CrossBtn = WinElements:FindFirstChild("Cross")
    if CrossBtn then
        local function safeDestroy()
            if Window.isDestroyed or isAnimatingWindow then return end
            isAnimatingWindow = true
        ...
        CrossBtn.MouseButton1Click:Connect(safeDestroy)
        CrossBtn.Activated:Connect(safeDestroy)
    end
    ...
    function Window:Resize(sizeX, sizeY, instant)
    ...
    local function endResize(input)
        ...
        if input and activeResizeInput and input ~= activeResizeInput then return end
        resizing = false
        isResizingGlobal = false
